...
// Quando o input de data perde o foco
document.getElementById('dataInicio').addEventListener('blur', verificarInputs);
document.getElementById('dataFim').addEventListener('blur', verificarInputs);

function verificarInputs() {
    // Se ambos os inputs de data têm um valor
    if (document.getElementById('dataInicio').value && document.getElementById('dataFim').value) {
        // Aqui você pode adicionar qualquer ação que queira executar quando ambos os campos de data estiverem preenchidos
    }
}

// Verifica os inputs inicialmente
verificarInputs();